...
export class ProductService {
  ...
  protected convertDateFromClient(product: IProduct): IProduct {
    const copy: IProduct = Object.assign({}, product, {
      dateCreated: product.dateCreated && product.dateCreated.isValid() ? product.dateCreated.toJSON() : undefined,
      lastUpdated: product.lastUpdated && product.lastUpdated.isValid() ? product.lastUpdated.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((product: IProduct) => {
        product.dateCreated = product.dateCreated ? moment(product.dateCreated) : undefined;
        product.lastUpdated = product.lastUpdated ? moment(product.lastUpdated) : undefined;
      });
    }
    return res;
  }
